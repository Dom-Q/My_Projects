...
namespace cs225 {
    ...
    double PNG::get_x_pixel(double longitude){
        return (180 + longitude) * x_scale;
    }

    double PNG::get_y_pixel(double latitude){
        return (90 - latitude) * y_scale;
    }

    double PNG::x_to_long(double pix_x){
        return (pix_x / x_scale) - 180;
    }

    double PNG::y_to_lat(double pix_y){
        return ((pix_y / y_scale) - 90)*-1;
    }
