...
int32_t read_dentry_by_name(const uint8_t *fname, dentry_t *dentry)
{
    ...
    for (i = 0; i < num_entries; i++){
        ...
        cmp_val = strncmp((const int8_t *)(S->d_entries[i].file_name), (const int8_t *)fname, len);
...
int32_t read_dentry_by_index(uint32_t index, dentry_t *dentry)
{
    // Validate index
    if (index < 0) return -1;
    if(index >= S->num_dentries){
        dentry->file_name[0] = '\0';
        return 0;
    }
    // Search by inode
...
data_block_t *get_dblock_base(superblock *sb_base)
{
    ...
    return (data_block_t *)((uint8_t *)sb_base + (KB_4 * num_blocks));
}
...
int32_t read_data(uint32_t inode, uint32_t offset, uint8_t *buf, uint32_t length)
{
    ...
    {
        // Validate pointer
        if (curr_node.data_block[i] == NULL)
            return -1;
